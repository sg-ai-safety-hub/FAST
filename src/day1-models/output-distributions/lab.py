...
# over the next token, something samples a token from that distribution, and the loop repeats.
# What looks like confidence or a refusal is a shape in that distribution plus a choice about
# how to read from it.
#
...
from fast.labs import distributions as lab
...
# token in the vocabulary. `lab.logits_for` runs the pass and hands back the last position's
# vector, the model's opinion about what comes next. None of it is a probability yet. Look at
# the range.
...
#
# Softmax turns logits into a distribution. Temperature divides the logits first: below 1 it
# sharpens the distribution toward the top token, above 1 it flattens it toward uniform.
#
...
#
# "The model is confident" is something you can put a number on. Shannon entropy over the
# next-token distribution, measured in bits: 0 when one token is certain, and *n* bits when
# the model is as undecided as a uniform choice among 2ⁿ tokens. Reach for it whenever you
# want to tell reciting apart from guessing.
...
# %% [markdown]
# Greedy decoding is repeatable. Anything above temperature 0 trades that repeatability for
# variety, and the trade is chosen at deployment, not fixed in the weights. Two teams running
# the same checkpoint at different temperatures ship models that behave differently.
...
#
# The mechanics: run the prompt and completion through together, take the log-softmax at every
# position, and sum the values for the completion's tokens only. Position *i*'s logits predict
# token *i+1*, so an off-by-one is the classic bug here. The check will catch it.
...
# questions with known answers and count how often the top-scored choice is right. The result
# is the accuracy you read in a model card.
#
